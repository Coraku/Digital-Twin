...
// State variables
  ...
  constant BatteryParams NMC_60kWh(
    R_int = 0.03,        
    Q_nom = 540000,     // C: 60kWh / 400V = 150Ah -> 150*3600
    beta_degrad = 0.10, 
    degradation_rate = 5e-8, 
    m_batt = 350,       // kg, 60kWh / 175 Wh/kg (175=pack level energy density)
    c_p = 900,          
    h = 8,             
    A = 3.0           
  );
  ...
  constant BatteryParams LFP_60kWh(
    R_int = 0.05,       
    Q_nom = 675000,     // C: 60kWh / 320V = 187.5Ah -> *3600
    beta_degrad = 0.05,
    degradation_rate = 2e-8, 
    m_batt = 415,    // kg,, 60kWh / 145 Wh/kg 
    c_p = 920,          
    h = 7,               
    A = 3.2             
  );
  ...
  constant BatteryParams SolidState_60kWh(
    R_int = 0.015,       // projected lower resistance -->solid electrolyte
    Q_nom = 540000,     // C: 60kWh / 400V = 150Ah -> *3600
    beta_degrad = 0.02, // Empirical: slowest degradation (theoretical)
    degradation_rate = 5e-9,
    m_batt = 240,       // kG, 60kWh / 250 Wh/kg (250=projected pack density)
    c_p = 850,          
    h = 10,             
    A = 2.5             // m2, more compact pack geometry
  );
  end BatteryTypes;
