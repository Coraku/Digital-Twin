...
protected
  ...
  Real torque_mot_max;
equation
  motor.omega = controller.V / KmPhi; //[rad/s]
  KmPhi = 60 / (mot_vel * 2 * pi);
  torque_mot_max = KmPhi * controller.I;
  torque_mot_raw = KmPhi * controller.V / R_a - KmPhi ^ 2 / R_a * motor.omega;
  //limit motor torque
  motor.tau = min(torque_mot_raw, torque_mot_max);
  annotation(
